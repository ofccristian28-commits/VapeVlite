  ...
  public class ModulesPart extends Component
{
    ...
    public ModulesPart(final Module mod, final Frame parent, final int offset) {
        ...
        opY += 2;
        if (!mod.getStrings().isEmpty()) {
            for (final StringValue str : mod.getStrings()) {
                this.subcomponents.add(new StringPart(str, this, opY));
                opY += 14;
            }
        }
    ...
    public void render() {
        ...
        if (!this.mod.getOptions().isEmpty() || !this.mod.getValues().isEmpty() || !this.mod.getStrings().isEmpty()) {
